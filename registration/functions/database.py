...
def validateClientEmailDB(dataObj, ip, device):
    try:
        cnx = connectToDatabase()
        mycursor = cnx.cursor()
        sql = "SELECT phonenumber,ip_address,device FROM registration WHERE email='{}' AND ip_address='{}' AND device='{}'".format(
...
def saveClientEmailDB(dataObj, ip, device, otp):
    try:
        cnx = connectToDatabase()
        mycursor = cnx.cursor()
        sql = "INSERT INTO registration(email,ip_address,device,email_otp) VALUES(%s,%s,%s,%s)"
        val = (dataObj['email'], ip, device, otp)
        mycursor.execute(sql, val)
        cnx.commit()
